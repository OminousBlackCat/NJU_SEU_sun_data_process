...
def RB_repair(imgData, sun_std, HofH, HofFe):
    ...
    stdx = np.zeros(H)
    sun_std[HofH:HofH + HofFe] = sun_std[260:260 + HofFe]
...
def DivFlat(imgData, flatData, HofH, HofFe):
    ...
    if offset < 0:
        offset *= -1
        flatHa[:, 0:W - offset] = flatHa[:, offset:W]
    else:
        flatHa[:, offset:W] = flatHa[:, 0:W - offset]
    imgFe = imgData[HofH:]
    flatFe = flatData[HofH:]
    ...
    if offset < 0:
        offset *= -1
        flatFe[:, 0:W - offset] = flatFe[:, offset:W]
    else:
        flatFe[:, offset:W] = flatFe[:, 0:W - offset]
    flatData[0:HofH] = flatHa
    flatData[HofH:] = flatFe
    return imgData / flatData


# 图像中值平滑操作
# 参数
def MedSmooth(imgData, winSize):
...
def entireWork(filename, darkDate, flatData, sun_std, kernel_size):
    # 读文件
    ...
    # 转为整型
    imgData = np.array(imgData, dtype=np.int16)
    # 滤波
    imgData = MedSmooth(imgData, kernel_size)
    return imgData
